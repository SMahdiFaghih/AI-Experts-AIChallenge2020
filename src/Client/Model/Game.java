...
public class Game implements World
{
    ...
    {
        long result = (this.turnTime + clientInitMessage.getGameConstants().getTurnTimeout() - System.currentTimeMillis());
        return (int) result;
    }
    ...
    {
        if (unit == null || path == null || cell == null || spell == null)
        {
            return;
        }
    ...
    {
        return clientInitMessage.getGameConstants().getTurnsToUpgrade() - clientTurnMessage.getCurrTurn() % clientInitMessage.getGameConstants().getTurnsToUpgrade();
    }

    @Override
    public int getRemainingTurnsToGetSpell()
    {
        return clientInitMessage.getGameConstants().getTurnsToSpell() - clientTurnMessage.getCurrTurn() % clientInitMessage.getGameConstants().getTurnsToSpell();
    }
    ...
    private int getFriendIdOfPlayer(int playerId)
    {
        ...
        if (playerId == getFirstEnemyId())
        {
            return getSecondEnemyId();
        }
        if (playerId == getSecondEnemyId())
        {
    ...
    private Path calcShortestPathToCell(int fromPlayerId, Cell cell)
    {
        ...
        if (path1 != null)
        {
            length1 = path1.getCells().indexOf(cell);
        }
        if (path2 != null)
        {
            length2 = path3.getCells().size() + path2.getCells().indexOf(cell);
        }
        if (length1 <= length2 && length1 < 100 * 1000)
        {
            return path1;
        }
        if (length1 > length2)
    ...
    {
        if (row >= initMessage.getMap().getRowNum() || row < 0)
        {
            return null;
        }
        if (col >= initMessage.getMap().getColNum() || col < 0)
        {
            return null;
        }
    ...
    private void calcCastAreaSpells()
    {
        for (Player player : players)
        {
            ...
            {
                if (castSpell instanceof CastAreaSpell && castSpell.getCasterId() == player.getPlayerId() && castSpell.isWasCastThisTurn())
                {
    ...
    private void calcCastUnitSpells()
    {
        for (Player player : players)
        {
            for (CastSpell castSpell : turnMessage.getCastSpells())
            {
                if (castSpell instanceof CastUnitSpell && castSpell.getCasterId() == player.getPlayerId() && castSpell.isWasCastThisTurn())
                {
    ...
    private void calcDiedUnits()
    {
        for (Player player : players)
        {
            ...
            {
                if (diedUnit.getPlayerId() == player.getPlayerId())
                {
                    diedUnits.add(diedUnit.castToUnit());
                }
            }
